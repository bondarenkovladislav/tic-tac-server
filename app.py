    ...
    if(token):
        if((not users.get(token)) and sid):
            if (len(users) == 1):
                ...
                game.setFirstUser(listUsers[0], decode_auth_token(list(users.keys())[0].decode('utf-8')))
                game.setSecondUser(listUsers[1], decode_auth_token(list(users.keys())[1].decode('utf-8')))
                for key in users:
